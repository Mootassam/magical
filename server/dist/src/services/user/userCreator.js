...
class UserCreator {
    ...
    get _status() {
        if (this.data.status &&
            !Array.isArray(this.data.status)) {
            return this.data.status;
        }
        else {
            const uniqueStatus = new Set(this.data.status);
            return uniqueStatus;
        }
    }
    get _emails() {
    ...
    _addOrUpdate(email) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!user) {
                user = yield userRepository_1.default.create(Object.assign({ email, 
                    // firstName: email.split('@')[0],
                    fullName: this.data.fullName }, this.data), Object.assign(Object.assign({}, this.options), { session: this.session }));
            }
            const isUserAlreadyInTenant = user.tenants.some((userTenant) => userTenant.tenant.id ===
                this.options.currentTenant.id);
            const tenantUser = yield tenantUserRepository_1.default.updateRoles(this.options.currentTenant.id, user.id, this._roles, Object.assign(Object.assign({}, this.options), { addRoles: true, session: this.session }), this._status);
